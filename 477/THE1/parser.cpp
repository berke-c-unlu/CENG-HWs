...
parser::Vec3f scalar(const parser::Vec3f & a, const float & s){
    parser::Vec3f result = {0,0,0};
...
void parser::Scene::loadFromXml(const std::string &filepath)
{
    ...
    stream << element->GetText() << std::endl;
    Vec3f vertex;
    Vec3f minPnt = {__FLT_MAX__,__FLT_MAX__,__FLT_MAX__};
    Vec3f maxPnt = {__FLT_MIN__,__FLT_MIN__,__FLT_MIN__};
    while (!(stream >> vertex.x).eof())
    {
        stream >> vertex.y >> vertex.z;

        // get min and max point of whole scene
        if(vertex.x < minPnt.x && vertex.y < minPnt.y && vertex.z < minPnt.z){
            minPnt = vertex;
        }
        if(vertex.x > maxPnt.x && vertex.y > maxPnt.y && vertex.z > maxPnt.z){
            maxPnt = vertex;
        }
    ...
    while (element)
    {
        ...
        triangle.indices.n = cross(add(c,scalar(b,-1)),add(a,scalar(b,-1)));
